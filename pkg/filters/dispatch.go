...
func WithDispatchByNats(next http.Handler, consul *consul.Consul, dispatchSvc *service.DispatchServer, currentZone string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requestInfo := RequestInfoFromHttpRequest(r)
		...
		if requestInfo.Zone == currentZone {
			r.URL.Path = strings.Replace(r.URL.Path, fmt.Sprintf("/zones/%s", requestInfo.Zone), "", 1)
			next.ServeHTTP(w, r)
			return
		}
		...
		targetConn := dispatchSvc.NewNatsConn(fmt.Sprintf("%s/%s", requestInfo.Zone, target.Service.ID), r.Header.Get(HeaderOperationID))
		defer targetConn.Close()
